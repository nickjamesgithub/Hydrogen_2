...
if experiment == "Chemical":
    ...
    for j in range(len(feature_list)):
        for k in range(len(feature_list)):
            # Compute normalized trajectories
            trajectory_j = feature_list[j] / np.sum(np.abs(feature_list[j]))
            trajectory_k = feature_list[k] / np.sum(np.abs(feature_list[k]))
