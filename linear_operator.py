...
alpha_grid = np.linspace(-10,500,30) # 30
beta_grid = np.linspace(-5,40,30) # 20

# Store optimal values for alpha & beta
optimal_alpha_list = []
optimal_beta_list = []
grid_length_list = []
age_c_median_list = []
...
for j in range(0,len(green_capacity_list)-1):
    ...
    optimal_alpha_list.append(alpha_opt)
    optimal_beta_list.append(beta_opt)

# Convert alpha and beta (operator parameters) to dataframes and concatentate
optimal_alpha_df = pd.DataFrame(optimal_alpha_list)
optimal_beta_df = pd.DataFrame(optimal_beta_list)
concat = pd.concat([optimal_alpha_df, optimal_beta_df], axis=1)
concat.to_csv("/Users/tassjames/Desktop/linear_operator_parameters.csv")
